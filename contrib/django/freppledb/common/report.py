...
class GridReport(View):
  ...
  def _generate_csv_data(reportclass, request, *args, **kwargs):
    ...
    for row in hasattr(reportclass, 'query') and reportclass.query(request, query) or query.values(*fields):
      ...
      if hasattr(row, "__getitem__"):
        writer.writerow([
          row[f] is None and ' ' or force_text(_localize(row[f], decimal_separator), encoding=encoding, errors='ignore', strings_only=True)
          for f in fields
          ])
      else:
        writer.writerow([
          getattr(row, f) is None and ' ' or force_text(_localize(getattr(row, f), decimal_separator), encoding=encoding, errors='ignore', strings_only=True)
          for f in fields
          ])
      # Return string
      yield sf.getvalue()
  ...
  def _generate_csv_data(reportclass, request, *args, **kwargs):
    ...
    if listformat:
      for row in query:
        ...
        if hasattr(row, "__getitem__"):
          fields = [
            row[f.name] is None and ' ' or force_text(row[f.name], encoding=encoding, errors='ignore')
            for f in reportclass.rows
            if f.name and not f.hidden
            ]
          fields.extend([ force_text(row['bucket'], encoding=encoding, errors='ignore') ])
          fields.extend([
            row[f[0]] is None and ' ' or force_text(_localize(row[f[0]], decimal_separator), encoding=encoding, errors='ignore')
            for f in reportclass.crosses
            ])
        else:
          fields = [
            getattr(row, f.name) is None and ' ' or force_text(getattr(row, f.name), encoding=encoding, errors='ignore')
            for f in reportclass.rows
            if f.name and not f.hidden
            ]
          fields.extend([ force_text(getattr(row, 'bucket'), encoding=encoding, errors='ignore') ])
          fields.extend([
            getattr(row, f[0]) is None and ' ' or force_text(_localize(getattr(row, f[0]), decimal_separator), encoding=encoding, errors='ignore')
            for f in reportclass.crosses
            ])
        # Return string
        writer.writerow(fields)
